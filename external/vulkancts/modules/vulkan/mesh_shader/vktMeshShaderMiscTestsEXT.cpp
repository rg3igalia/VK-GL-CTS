...
{
public:
    ClipCase(tcu::TestContext &testCtx, const std::string &name, ParamsPtr params, bool useClipPlanes,
             bool useProvokingVertexLast, bool useMultiview)
        : MeshShaderMiscCase(testCtx, name, std::move(params))
...
{
public:
    ClipInstance(Context &context, const MiscTestParams *params, bool useClipPlanes, bool useProvokingVertexLast,
                 bool useMultiview)
...
void ClipCase::initPrograms(vk::SourceCollections &programCollection) const
{
    ...
    frag << "#version 450\n"
         ...
         << "                        customAttribute1.w == 3.0);\n";
    if (m_useProvokingVertexLast)
        frag << "    bool goodCustom2 = (gl_PrimitiveID == 1000 &&  customAttribute2 == 2.0)  || (gl_PrimitiveID == "
                "1001 && customAttribute2 == 1.0);\n";
    else
        frag << "    bool goodCustom2 = (gl_PrimitiveID == 1000 &&  customAttribute2 == 1.0)  || (gl_PrimitiveID == "
                "1001 && customAttribute2 == 2.0);\n";
    frag << "    bool goodCustom3 = (customAttribute3 == 3 || customAttribute3 == 4);\n"
         << "    bool goodCustom4 = ((gl_PrimitiveID == 1000 && customAttribute4 == uvec4(100, 101, 102, 103)) ||\n"
         << "                        (gl_PrimitiveID == 1001 && customAttribute4 == uvec4(200, 201, 202, 203)));\n"
         << "    bool goodCustom5 = ((gl_PrimitiveID == 1000 && customAttribute5 == 6.0) ||\n"
         << "                        (gl_PrimitiveID == 1001 && customAttribute5 == 7.0));\n"
    ...
    if (m_useClipDistances)
        pvdDataDeclStream << "    float clipDistances[4];\n";
    else
        pvdDataDeclStream << "    float unused[4];\n";
    pvdDataDeclStream << "    vec4 custom1[4];\n"
                      << "    float custom2[4];\n"
                      << "    int custom3[4];\n";
    const auto pvdDataDecl = pvdDataDeclStream.str();
    ...
    mesh << "#version 450\n"
         ...
         << "    float gl_PointSize;\n";
    if (m_useClipDistances)
        mesh << "    float gl_ClipDistance[1];\n";
    mesh << "} gl_MeshVerticesEXT[];\n"
         << "\n"
         << "layout (location=0) out vec4 customAttribute1[];\n"
         << "layout (location=1) out flat float customAttribute2[];\n"
         << "layout (location=2) out int customAttribute3[];\n"
         ...
         << "\n";
    if (m_useClipDistances)
        mesh << "    // Remove geometry on the right side.\n"
             << "    gl_MeshVerticesEXT[0].gl_ClipDistance[0] = " << meshPvdPrefix << ".clipDistances[0]; // 1.0\n"
             << "    gl_MeshVerticesEXT[1].gl_ClipDistance[0] = " << meshPvdPrefix << ".clipDistances[1]; //-1.0\n"
             << "    gl_MeshVerticesEXT[2].gl_ClipDistance[0] = " << meshPvdPrefix << ".clipDistances[2]; // 1.0\n"
             << "    gl_MeshVerticesEXT[3].gl_ClipDistance[0] = " << meshPvdPrefix << ".clipDistances[3]; //-1.0\n"
             << "    \n";
    mesh << "    gl_PrimitiveTriangleIndicesEXT[0] = uvec3(0, 1, 2);\n"
         << "    gl_PrimitiveTriangleIndicesEXT[1] = uvec3(2, 3, 1);\n"
         << "\n"
         << "    gl_MeshPrimitivesEXT[0].gl_PrimitiveID = " << meshPpdPrefix << ".primitiveIds[0]; //1000\n"
         << "    gl_MeshPrimitivesEXT[1].gl_PrimitiveID = " << meshPpdPrefix << ".primitiveIds[1]; //1001\n"
    ...
    if (taskShader)
    {
        ...
        task << "#version 450\n"
             ...
             << "\n";
        if (m_useClipDistances)
            task << "    td.clipDistances[0] = pvd.clipDistances[0];\n"
                 << "    td.clipDistances[1] = pvd.clipDistances[1];\n"
                 << "    td.clipDistances[2] = pvd.clipDistances[2];\n"
                 << "    td.clipDistances[3] = pvd.clipDistances[3];\n"
                 << "\n";
        task << "    td.custom1[0] = pvd.custom1[0];\n"
             << "    td.custom1[1] = pvd.custom1[1];\n"
             << "    td.custom1[2] = pvd.custom1[2];\n"
             << "    td.custom1[3] = pvd.custom1[3];\n"
             << "\n"
...
tcu::TestStatus ClipInstance::iterate()
{
    ...
    Move<VkImageView> colorView =
        makeImageView(vkd, device, colorImage.get(),
                      m_useMultiview ? VK_IMAGE_VIEW_TYPE_2D_ARRAY : VK_IMAGE_VIEW_TYPE_2D, imageFormat, colorSRR);
    ...
    if (m_useMultiview)
    {
        verificationBuffer1 = de::MovePtr<BufferWithMemory>(
            new BufferWithMemory(vkd, device, alloc, verificationBufferInfo, MemoryRequirement::HostVisible));
    ...
    const PerVertexData &perVertexData =
        (m_useClipDistances ? perVertexWithClipDistancesData : perVertexClippedGeometryData);
    ...
    // Framebuffer.
    const auto framebuffer =
        makeFramebuffer(vkd, device, renderPass.get(), 1u, &colorView.get(), imageExtent.width, imageExtent.height);
    ...
    provokingVertexInfo.provokingVertexMode = (m_useProvokingVertexLast ? VK_PROVOKING_VERTEX_MODE_LAST_VERTEX_EXT :
    ...
    const auto pipeline =
        makeGraphicsPipeline(vkd, device, pipelineLayout.get(), taskShader.get(), meshShader.get(), fragShader.get(),
    ...
    vkd.cmdPipelineBarrier(cmdBuffer, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0u,
                           0u, nullptr, 0u, nullptr, 1u, &preCopyBarrier0);

    vkd.cmdCopyImageToBuffer(cmdBuffer, colorImage.get(), VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                             verificationBuffer0.get(), 1u, &copyRegion0);
    if (m_useMultiview)
    {
        const auto copyRegion1 = makeBufferImageCopy(imageExtent, colorSRL1);
        vkd.cmdCopyImageToBuffer(cmdBuffer, colorImage.get(), VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                                 verificationBuffer1->get(), 1u, &copyRegion1);
    }
    vkd.cmdPipelineBarrier(cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_HOST_BIT, 0u, 1u,
                           &postCopyBarrier, 0u, nullptr, 0u, nullptr);
            ...
            for (int j = 0; j < 2; ++j)
                for (int k = 0; k < 2; ++k)
                    for (int l = 0; l < 2; ++l)
                    {
                        ...
                        const auto name                   = std::string(useClipPlanes ? "clip_plane" : "clip_geom") +
                                          (useTaskShader ? "_and_task_shader" : "") +
                                          (useProvokingVertexLast ? "_provoking_last" : "") +
                                          (useMultiview ? "_multiview" : "");
                        ...
                        miscTests->addChild(new ClipCase(testCtx, name, std::move(paramsPtr), useClipPlanes,
                                                         useProvokingVertexLast, useMultiview));
                    }
    }
